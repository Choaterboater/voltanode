...
export const strategies: Strategy[] = [
  {
    id: 'strat-1',
    name: 'Momentum',
    description: 'Trend-following strategy that enters positions in the direction of established price momentum. Uses RSI and moving average crossovers for confirmation.',
    category: 'Trend',
    winRate: 62.4,
    avgReturn: 3.8,
    sharpeRatio: 1.72,
    ...
    tradesCount: 348,
    params: [
      { key: 'lookback', label: 'Lookback Period', type: 'number', default: 20, min: 5, max: 100, step: 1 },
      { key: 'threshold', label: 'Momentum Threshold', type: 'number', default: 0.02, min: 0.001, max: 0.1, step: 0.001 },
      { key: 'stopLoss', label: 'Stop Loss %', type: 'number', default: 3, min: 0.5, max: 10, step: 0.1 },
    ],
  },
  {
    id: 'strat-2',
    name: 'Mean Reversion',
    description: 'Counter-trend strategy that trades price deviations from statistical mean. Uses Bollinger Bands and z-score for entry signals.',
    ...
    tradesCount: 512,
    params: [
      { key: 'window', label: 'MA Window', type: 'number', default: 50, min: 10, max: 200, step: 1 },
      { key: 'deviation', label: 'Std Dev Entry', type: 'number', default: 2, min: 1, max: 4, step: 0.1 },
      { key: 'takeProfit', label: 'Take Profit %', type: 'number', default: 1.5, min: 0.5, max: 5, step: 0.1 },
    ],
  },
  {
    id: 'strat-3',
    name: 'Grid Trading',
    description: 'Systematic grid strategy that places buy and sell orders at regular price intervals. Profits from ranging markets and volatility.',
    category: 'Systematic',
    winRate: 71.2,
    avgReturn: 1.8,
    sharpeRatio: 1.88,
    ...
    tradesCount: 1247,
    params: [
      { key: 'gridLevels', label: 'Grid Levels', type: 'number', default: 10, min: 4, max: 50, step: 1 },
      { key: 'gridSize', label: 'Grid Size %', type: 'number', default: 1, min: 0.2, max: 5, step: 0.1 },
      { key: 'investment', label: 'Capital per Grid', type: 'number', default: 100, min: 10, max: 1000, step: 10 },
    ],
  },
  {
    id: 'strat-4',
    name: 'Breakout',
    description: 'Volatility expansion strategy that enters on price breakouts from consolidation ranges. Uses ATR for dynamic position sizing.',
    category: 'Volatility',
    winRate: 54.3,
    avgReturn: 5.2,
    sharpeRatio: 1.35,
    ...
    tradesCount: 186,
    params: [
      { key: 'rangePeriod', label: 'Range Period', type: 'number', default: 20, min: 5, max: 60, step: 1 },
      { key: 'breakoutFactor', label: 'Breakout Factor', type: 'number', default: 1.5, min: 1, max: 3, step: 0.1 },
      { key: 'atrMultiplier', label: 'ATR Multiplier', type: 'number', default: 2, min: 1, max: 5, step: 0.1 },
    ],
  },
  {
    id: 'strat-5',
    name: 'MACD',
    description: 'Classic MACD-based strategy with signal line crossovers. Includes histogram divergence detection for early signals.',
    category: 'Indicator',
    winRate: 59.8,
    avgReturn: 2.9,
    sharpeRatio: 1.52,
    ...
    tradesCount: 423,
    params: [
      { key: 'fastEMA', label: 'Fast EMA', type: 'number', default: 12, min: 5, max: 30, step: 1 },
      { key: 'slowEMA', label: 'Slow EMA', type: 'number', default: 26, min: 15, max: 50, step: 1 },
      { key: 'signalEMA', label: 'Signal EMA', type: 'number', default: 9, min: 5, max: 20, step: 1 },
    ],
  },
  {
    id: 'strat-6',
    name: 'Arbitrage',
    description: 'Cross-exchange arbitrage that exploits price discrepancies. Requires accounts on multiple exchanges.',
    category: 'Systematic',
    winRate: 85.4,
    avgReturn: 0.8,
    sharpeRatio: 2.34,
    ...
    tradesCount: 2156,
    params: [
      { key: 'minSpread', label: 'Min Spread %', type: 'number', default: 0.15, min: 0.05, max: 1, step: 0.01 },
      { key: 'maxSlippage', label: 'Max Slippage %', type: 'number', default: 0.1, min: 0.01, max: 0.5, step: 0.01 },
      { key: 'positionSize', label: 'Position Size', type: 'number', default: 500, min: 100, max: 5000, step: 100 },
    ],
  },
  {
    id: 'strat-7',
    name: 'ML Ensemble',
    description: 'Machine learning ensemble using Random Forest and LSTM models. Trained on technical indicators and order flow data.',
    category: 'AI/ML',
    winRate: 64.1,
    avgReturn: 4.2,
    sharpeRatio: 1.95,
    ...
    tradesCount: 267,
    params: [
      { key: 'modelConfidence', label: 'Confidence Threshold', type: 'number', default: 0.65, min: 0.5, max: 0.95, step: 0.01 },
      { key: 'featureWindow', label: 'Feature Window', type: 'number', default: 30, min: 10, max: 100, step: 1 },
      { key: 'retrainInterval', label: 'Retrain (hours)', type: 'number', default: 24, min: 6, max: 168, step: 6 },
    ],
  },
];
