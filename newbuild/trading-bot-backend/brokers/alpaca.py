...
class AlpacaBroker(BrokerAdapter):
    ...
    def place_order(self, order: Order) -> FillResult:
        ...
        type_map = {
            ...
            OrderType.TAKE_PROFIT: "trailing_stop",
        ...
        # Alpaca returns the order object; check if filled
        status = data.get("status")
        if status not in ("filled", "partially_filled"):
            raise BrokerConnectionError(
                f"Alpaca order not immediately filled (status={status}). "
                "Use market orders or poll for fills."
            )
