...
class PaperTradingEngine:
    ...
    def execute_order(self, order: Order, current_price: float) -> FillResult | None:
        ...
        # Use modified order if risk manager adjusted it
        if risk_result.order is not None:
    ...
    def get_trade_history(self, account_id: str | None = None) -> List[Trade]:
        ...
        if account_id:
            ...
            if portfolio:
                return [t for t in self._trades if t.id in [tr["id"] for tr in portfolio.get_trade_history()]]
