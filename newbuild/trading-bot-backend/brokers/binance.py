...
class BinanceBroker(BrokerAdapter):
    ...
    def place_order(self, order: Order) -> FillResult:
        ...
        # Binance limit orders may be open (status NEW) — this adapter returns
        # a FillResult only if fully filled, otherwise raises.
        if data.get("status") not in ("FILLED", "PARTIALLY_FILLED"):
            raise BrokerConnectionError(
                f"Order not immediately filled (status={data.get('status')}). "
                "Use MARKET orders or polling for limit fills."
            )
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            self._request(
                "DELETE",
                "/api/v3/order",
                params={"symbol": "BTCUSDT", "orderId": order_id},  # symbol required — best effort
                signed=True,
            )
            return True
