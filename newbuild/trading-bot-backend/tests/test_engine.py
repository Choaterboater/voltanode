...
class TestOrderExecution:
    ...
    def test_market_order_fill(self, engine: PaperTradingEngine) -> None:
        ...
        order = Order.market("BTC", OrderSide.BUY, 1.0, account_id="default")
    ...
    def test_limit_order_fill(self, engine: PaperTradingEngine) -> None:
        ...
        order = Order.limit("ETH", OrderSide.BUY, 2.0, price=3000.0, account_id="default")
        engine.submit_order(order)
    ...
    def test_short_position_pnl(self, engine: PaperTradingEngine) -> None:
        """Test short position P&L - profit when price drops."""
        ...
        # Price drops to 90 - short profits
        engine.on_tick(TickData(symbol="TEST", price=90.0))
        pos = portfolio.get_position("TEST")
        assert pos.unrealized_pnl == 50.0  # (100-90) * 5
