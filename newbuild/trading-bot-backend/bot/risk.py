...
class ExposureMonitor:
    ...
    def check(
        ...
        for pos in portfolio.get_all_positions():
            # Simplified: check against a notional limit
            if pos.market_value > 0:
                # We would need total equity here; simplified check
                pass

        # Check per-asset exposure
        max_asset_pct = limits.get("max_exposure_per_asset_pct", 0.30)
        for symbol in set(p.symbol for p in portfolio.get_all_positions()):
